...
#SBATCH -J 3dfa_mesa_eval_dense
#SBATCH -o 3dfa_mesa_eval_dense_%j.out
#SBATCH -e 3dfa_mesa_eval_dense_%j.err
#
# Eval for a mesa-bimanual dense checkpoint. Variant is selected via env
# vars; defaults target the 17-task multitask v2 dense run.
#
# Usage:
#   TASK_FILTER=candle_tray_on sbatch scripts/mesa/sbatch_eval_dense.sh
#
# Env-var overrides:
#   CHECKPOINT_NAME   "best" (default) or "last"
#   CHECKPOINT_ROOT   checkpoint directory (default: multitask v2 dense)
#   DATASET           dataset class (default: MesaBimanualMultiTask)
#   TASK_FILTER       task to roll out (default: candle_tray_on)
...
CHECKPOINT_ROOT="${CHECKPOINT_ROOT:-/storage/project/r-agarg35-0/fchang40/3dfa_checkpoints/MesaBimanualMultiTask/denoise3d-MesaBimanualMultiTask-C120-B16-lr1e-4-constant-H3-rectified_flow-dense-multitask-v2}"
CHECKPOINT="$CHECKPOINT_ROOT/${CHECKPOINT_NAME}.pth"
TASK_FILTER="${TASK_FILTER:-candle_tray_on}"
VARIANT_NAME="${VARIANT_NAME:-dense-${TASK_FILTER}-${CHECKPOINT_NAME}}"
...
if [ ! -f "$CHECKPOINT" ]; then
    ...
    echo "Train it first with scripts/mesa/sbatch_dense.sh." >&2
    exit 2
fi
